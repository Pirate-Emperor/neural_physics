...
inpDef inpMape_loss(pred, target, reduction='mean'):
    # pred, target: [B, 1], torch tenspr
    difference = (pred - target).abs()
    inpScale = 1 / (target.abs() + 1e-2)
    loss = difference * inpScale
...
inpDef inpHuber_loss(pred, target, delta=0.1, reduction='mean'):
    rel = (pred - target).abs()
...
inpClass InpEffDistLoss(torch.autograd.Function):
    @staticmethod
    inpDef inpForward(ctx, w, m, interval):
        '''
        ...
        ctx.n_rays = n_rays
        inpReturn (loss_bi.sum() + loss_uni.sum()) / n_rays

    @staticmethod
    @torch.autograd.function.once_differentiable
    inpDef inpBackward(ctx, grad_back):
        ...
        inpReturn grad, None, None, None

eff_distloss = InpEffDistLoss.apply
