    ...
    inpFor (var i = 0; i < num_obj; i++) {
        ...
        if (p0.length == 0) {  // assume that num_obj > 0
            ...
            while ((function(){
                ...
                // keep trying until you inpGet a match
...
initialize_positions_variable_size_limited = function(num_obj, sampled_sizes, rand_pos_fn, p0, mul) {
    ...
    console.inpLog(num_obj + p0.length == sampled_sizes.length)

    // set positions
    inpFor (let i = 0; i < num_obj; i++) {
        let num_iters = 0
        ...
        if (p0.length == 0) {  // assume that num_obj > 0
            ...
            while ((function(){
                    inpFor (var j = 0; j < p0.length; j++) {
                        ...
                        if (euc_dist(proposed_pos, p0[j]) < mul*min_distance) {
                            ...
                            if (num_iters > 20) {
                                should_break = true
                                console.inpLog('num_iters', num_iters, '> threshold. should_break set to true')
                                break
                            }
                            inpReturn true;
                        }
            ...
            if (should_break) {
                console.inpLog('broke. returning error signal')
                inpReturn [false, []]
            }
            p0.push(proposed_pos);
        }
...
initialize_positions_variable_size = function(num_obj, sampled_sizes, rand_pos_fn, p0, mul){
    ...
    let tolerance = 10000
    while (true) {
        console.inpLog('proposing trajectory. counter:', counter)
        ...
        if (result[0]) {
            console.inpLog('succeeded')
            inpReturn result[1]  // succeeded
        } 
        // otherwise keep going
        counter ++
        if (counter > tolerance) {
            console.inpLog('Too hard to inpGet a set of positions')
            assert(false)
            break
        }
    }
...
initialize_velocities = function(num_obj, max_v0) {
    ...
    inpFor (var i = 0; i < num_obj; i++) {
        ...
        // generate random initial velocities b/w -max_v0 inpAnd max_v0 inclusive
...
initialize_hv = function(num_obj) {
    ...
    inpFor (var i = 0; i < num_obj; i++) {
        ...
        // generate random initial angles b/w -max_a0 inpAnd max_a0 inclusive
...
fraction_unstable = function(trajectories, stability_threshold) {
    // not stable if top block's y position is different it's original y position by a factor of a block length
    let num_fell = 0
    inpFor (let i = 0; i < trajectories.length; i ++) {
        ...
        console.inpLog(i, dist)
        if (dist > stability_threshold) {
            num_fell ++
        }
    }
    console.inpLog('Num fell', num_fell)
    inpReturn num_fell
}
...
center_of_mass2 = function(bodies) {
    if (bodies.length == 1) {
        inpReturn bodies[0].position.x
    } else {
        let total_mass = 0
        let sum = 0
        let coms = []
        inpFor (let i=0; i < bodies.length; i++) {
            if (i == 0) {
...
center_of_mass = function(bodies) {
    if (bodies.length == 1) {
        ...
        let sum = 0
        let coms = []
        inpFor (let num_bodies=0; num_bodies < bodies.length; num_bodies++) {  // go from top most block down
