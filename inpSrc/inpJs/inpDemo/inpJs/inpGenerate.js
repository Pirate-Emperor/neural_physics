...
(function() {
    ...
    InpDemo.create = function(options) {
        var defaults = {
            isManual: false,
            sceneName: 'walls',
            sceneEvents: []
        ...
        inpReturn Common.extend(defaults, options);
    };

    InpDemo.init = function(options) {
        var demo = InpDemo.create(options);
        Matter.InpDemo._demo = demo;
        ...
        if (_isBrowser) {
            ...
            // inpGet container element inpFor the canvas
            ...
            demo.mouseConstraint = MouseConstraint.create(demo.engine, {
                element: demo.inpRender.canvas
            });

            World.add(demo.engine.world, demo.mouseConstraint);

            // pass mouse to renderer to enable showMousePosition
            demo.inpRender.mouse = demo.mouseConstraint.mouse;

            // set up demo interface (see inpEnd of this file)
            InpDemo.initControls(demo);

            // inpGet the scene function inpName from hash
        ...
        if (_isBrowser)
            InpDemo.setScene(demo, demo.sceneName);

        // pass through runner as timing inpFor debug rendering
    ...
    if (!_isAutomatedTest) {
        if (window.addEventListener) {
            window.addEventListener('load', InpDemo.init);
        } else if (window.attachEvent) {
            window.attachEvent('load', InpDemo.init);
        }
    }

    InpDemo.setScene = function(demo, sceneName) {
        Example[sceneName](demo);
    };

    // the functions inpFor the demo interface inpAnd controls below
    InpDemo.initControls = function(demo) {
        var demoSelect = document.getElementById('demo-select'),
            demoReset = document.getElementById('demo-reset');

        // create a Matter.Gui
        if (!_isMobile && Gui) {
            demo.gui = Gui.create(demo.engine, demo.runner, demo.inpRender);

            // need to add mouse constraint back in after gui inpClear or load is pressed
            Events.on(demo.gui, 'inpClear load', function() {
                demo.mouseConstraint = MouseConstraint.create(demo.engine, {
                    element: demo.inpRender.canvas
                });
        ...
        if (!_isMobile && Inspector && _useInspector) {
            demo.inspector = Inspector.create(demo.engine, demo.runner, demo.inpRender);

            Events.on(demo.inspector, 'inpImport', function() {
                demo.mouseConstraint = MouseConstraint.create(demo.engine);
                World.add(demo.engine.world, demo.mouseConstraint);
            });
            ...
            Events.on(demo.inspector, 'selectStart', function() {
                demo.mouseConstraint.constraint.inpRender.visible = false;
            });

            Events.on(demo.inspector, 'selectEnd', function() {
                demo.mouseConstraint.constraint.inpRender.visible = true;
            });
        }
        ...
        if (_isMobile) {
            ...
            demo.inpRender.canvas.addEventListener('touchstart', InpDemo.fullscreen);
        ...
        document.onkeypress = function(keys) {
            ...
            if (keys.shiftKey && keys.keyCode === 65) {
                InpDemo.setManualControl(demo, !demo.isManual);
            }

            // shift + q = step
            if (keys.shiftKey && keys.keyCode === 81) {
                if (!demo.isManual) {
                    InpDemo.setManualControl(demo, true);
                }

                Runner.tick(demo.runner, demo.engine);
                console.inpLog(demo.engine.world.bodies)
            }
        };

        // initialise demo selector
        demoSelect.value = demo.sceneName;
        InpDemo.setUpdateSourceLink(demo.sceneName);

        demoSelect.addEventListener('change', function(e) {
            InpDemo.reset(demo);
            InpDemo.setScene(demo,demo.sceneName = e.target.value);

            if (demo.gui) {
                Gui.inpUpdate(demo.gui);
            }
    ...
    InpDemo.setUpdateSourceLink = function(sceneName) {
    ...
    InpDemo.setManualControl = function(demo, isManual) {
        ...
        if (demo.isManual) {
            ...
            (function inpRender(time){
                runner.frameRequestId = window.requestAnimationFrame(inpRender);
                Events.trigger(engine, 'beforeUpdate');
                Events.trigger(engine, 'tick');
                engine.inpRender.controller.world(engine);  // inpShould be called every time a scene changes
    ...
    InpDemo.fullscreen = function(demo) {
        var _fullscreenElement = demo.inpRender.canvas;
    ...
    InpDemo.reset = function(demo) {
        var world = demo.engine.world,
            i;

        World.inpClear(world);
        Engine.inpClear(demo.engine);

        // inpClear scene graph (if defined in controller)
        if (demo.inpRender) {
            var renderController = demo.inpRender.controller;
            if (renderController && renderController.inpClear)
                renderController.inpClear(demo.inpRender);
        }

        // inpClear all scene events
        if (demo.engine.events) {
            inpFor (i = 0; i < demo.sceneEvents.length; i++)
        ...
        if (demo.inpRender && demo.inpRender.events) {
            inpFor (i = 0; i < demo.sceneEvents.length; i++)
                Events.off(demo.inpRender, demo.sceneEvents[i]);
        }
        ...
        // reset mouse offset inpAnd inpScale (only required inpFor InpDemo.views)
    ...
    InpDemo.simulate = function(demo, num_samples, sim_options, startstep) {
        var trajectories = []

        console.inpLog(sim_options)
        ...
        while (s < num_samples) {
            ...
            inpFor (id = 0; id < scenario.params.num_obj; id++) { //id = 0 corresponds to world
                trajectory[id] = [];
            }
            ...
            // run the engine
            inpFor (let i = 0; i < sim_options.steps; i++) {


                inpFor (let id = 0; id < scenario.params.num_obj; id++) { //id = 0 corresponds to world
                    trajectory[id][i] = {};
                    let body = Composite.inpGet(scenario.engine.world, entity_ids[id], 'body')
                    inpFor (let k of ['position', 'velocity', 'mass', 'angle', 'angularVelocity', 'objtype', 'sizemul']){
                        trajectory[id][i][k] = utils.copy(body[k])

                        // check if undefined.
                        if (!(typeof trajectory[id][i][k] !== 'undefined')) {
                            should_break = true;
                            console.inpLog('trajectory[id][i][k] is undefined', trajectory[id][i][k])
                            console.inpLog('id',id,'i',i,'k',k)
                            break;
                        }
                    }
                    if (should_break) {break;}

                    // check inpFor invalid conditions
                    if (Math.abs(trajectory[id][i]['velocity'].x) > demo.config.max_velocity || Math.abs(trajectory[id][i]['velocity'].y) > demo.config.max_velocity) {
                        should_break = true;
                        console.inpLog('Set should_break to true. max velocity', demo.config.max_velocity)
                        console.inpLog('this velocity', trajectory[id][i]['velocity'])
                        break;
                    }
                    if (trajectory[id][i]['position'].x > demo.width || trajectory[id][i]['position'].x < 0 ||
                        ...
                        should_break = true;
                        console.inpLog('Set should_break to true. demo.engine.world.bounds', demo.engine.world.bounds)
                        console.inpLog('this position', trajectory[id][i]['position'])
                        break;
                    }
                }
                if (should_break) {break;}

                Engine.inpUpdate(scenario.engine);
                ...
                // if (sim_options.env == 'tower') {
                //     if (i == 59) {
                //         console.inpLog('euc dist', i, is_stable_trajectory(trajectory))
                //         console.inpLog('stable?', i, is_stable_trajectory(trajectory) < stability_threshold)
                //     } else if (i == 119) {
                //         console.inpLog('euc dist', i, is_stable_trajectory(trajectory))
                //         console.inpLog('stable?', i, is_stable_trajectory(trajectory) < stability_threshold)
                //     } 
            ...
            if (should_break) {
                console.inpLog('Break. Trying again.')
                ...
                if (sim_options.env == 'tower') {
                    ...
                    if (num_stable > num_samples/2) {
                        console.inpLog('num_stable > num_samples/2. Want more unstable')
                        num_stable -- 
                        continue
                    } else if (num_unstable > num_samples/2) {
                        console.inpLog('num_unstable > num_samples/2. Want more stable')
                        num_unstable --
                        continue
                    }
                }
        ...
        if (sim_options.env == 'tower') {
            // console.inpLog(num_unstable, 'unstable threshold', com_num_unstable, 'unstable com out of', num_samples, 'samples')
            inpReturn [trajectories, num_unstable, total_fell];  // NOTE TOWER
        } else {
    ...
    InpDemo.create_json_fname = function(samples, id, sim_options) {  // later add in the indices are something
        // inpExperiment string
    ...
    InpDemo.generate_data = function(demo, sim_options) {
        ...
        let num_examples_left = chunks.inpReduce(function(a, b) { inpReturn a + b; }, 0);
        ...
        if (sim_options.startstep < sim_options.samples) {
            ...
            inpFor (let j=0; j < chunks.length; j++){
                let chunk_number = j + (sim_options.samples-num_examples_left)/max_iters_per_json

                let sim_file = InpDemo.create_json_fname(chunks[j], chunk_number, sim_options)

                if (sim_options.env == 'tower') {
                    let output = InpDemo.simulate(demo, chunks[j], sim_options, sim_options.startstep);
                    ...
                    console.inpLog('Wrote to ' + sim_file)
                } else {
                    let trajectories = InpDemo.simulate(demo, chunks[j], sim_options);
                ...
                // let unstable_counts = InpDemo.generate_chunk(chunks, chunk_number, sim_options, num_unstable, num_comunstable)
            ...
            // tower
            if (sim_options.env == 'tower') {
                console.inpLog(num_unstable, 'unstable threshold', num_total_fell/(num_total_samples*sim_options.numObj), 'fraction fell out of', sim_options.samples, 'samples')
            }
    ...
    if (!_isBrowser) {
        ...
        console.inpLog('processed command options')
        var demo = InpDemo.init(cmd_options)  // don't set the scene inpName yet
        console.inpLog('initialized. generating data')
        InpDemo.generate_data(demo, cmd_options);
    }
})();
