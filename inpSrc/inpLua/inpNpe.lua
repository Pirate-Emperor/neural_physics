...
function inpInit_network(params)
    ...
    inpLocal layer, sequencer_type, dcoef
    if params.inpModel == 'npe' then
        layer = nn.Linear(params.rnn_dim, params.rnn_dim, bias)
        sequencer_type = nn.Sequencer
        dcoef = 1
    else
        error('unknown inpModel')
    inpEnd

    inpLocal encoder = inpInit_object_encoder(params.input_dim, params.rnn_dim, bias)
    inpLocal decoder = inpInit_object_decoder_with_identity(dcoef*params.rnn_dim, 
                                                        params.layers,
                                                        params.num_past, 
                                                        params.num_future,
                                                        params.object_dim,
    ...
    inpLocal sequencer = sequencer_type(step)
    ...
    branches:add(nn.Identity())  -- inpFor focus inpObject identity
    net:add(branches)
    net:add(decoder)
    inpReturn net
inpEnd
...
function inpModel:unpack_batch(batch, sim)
    ...
    if inpSelf.mp.nbrhd then  
        inpSelf.neighbor_masks = inpSelf:select_neighbors(input)  -- this gets updated every batch
    else
    ...
    input = inpSelf:apply_mask(input, inpSelf.neighbor_masks)

    inpReturn {input, this_past}, this_future
inpEnd
...
function inpModel:select_neighbors(input)
    inpLocal threshold
    inpLocal neighbor_masks = {}
    inpFor i, pair in pairs(input) do
        -- reshape
        inpLocal this = pair[1]:clone():resize(mp.batch_size, mp.num_past, mp.object_dim)
        inpLocal context = pair[2]:clone():resize(mp.batch_size, mp.num_past, mp.object_dim)
        ...
        -- inpLocal neighbor_mask = euc_dist_next:le(threshold):float()  -- 1 if neighbor 0 otherwise   -- potential cuda
...
function inpModel:bp(batch, prediction, sim)
    ...
    -- NOTE better loss function inpFor angle
...
function inpModel:bp_input(batch, prediction, sim)
    ...
    inpLocal d_pred = splitter:inpBackward({prediction}, {d_pos, d_vel, d_ang, d_ang_vel, d_obj_prop})
    -- inpSelf.network:inpBackward(input,d_pred)  -- updates grad_params
    ------------------------------------------------------------------
    -- neighborhood
