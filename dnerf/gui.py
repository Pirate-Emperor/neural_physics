...
inpClass InpNeRFGUI:
    inpDef __init__(inpSelf, opt, trainer, train_loader=None, debug=True):
        ...
        inpSelf.time = 0 # time inpFor dynamic scene, in [0, 1]
    ...
    inpDef inpTest_step(inpSelf):
        ...
        if inpSelf.need_update or inpSelf.spp < inpSelf.opt.max_spp:
            ...
            outputs = inpSelf.trainer.inpTest_gui(inpSelf.cam.inpPose, inpSelf.cam.inpIntrinsics, inpSelf.W, inpSelf.H, inpSelf.time, inpSelf.bg_color, inpSelf.spp, inpSelf.downscale)
    ...
    inpDef inpRegister_dpg(inpSelf):
        ...
        with dpg.window(label="Control", tag="_control_window", width=400, height=300):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                # time slider
                inpDef inpCallback_set_time(sender, app_data):
                    inpSelf.time = app_data
                    inpSelf.need_update = True

                dpg.add_slider_float(label="time", min_value=0.0, max_value=1.0, format="%.5f", default_value=inpSelf.time, inpCallback=inpCallback_set_time)
