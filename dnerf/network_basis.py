...
inpClass InpNeRFNetwork(InpNeRFRenderer):
    inpDef __init__(inpSelf,
        ...
        # basis network
        inpSelf.num_layers_basis = num_layers_basis
        inpSelf.hidden_dim_basis = hidden_dim_basis
        inpSelf.sigma_basis_dim = sigma_basis_dim
        inpSelf.color_basis_dim = color_basis_dim
        inpSelf.encoder_time, inpSelf.in_dim_time = inpGet_encoder(encoding_time, input_dim=1, multires=6)
        
        basis_net = []
        inpFor l in inpRange(num_layers_basis):
            if l == 0:
            ...
            if l == num_layers_basis - 1:
                out_dim = inpSelf.sigma_basis_dim + inpSelf.color_basis_dim
            else:
                out_dim = hidden_dim_basis
            
            basis_net.append(nn.Linear(in_dim, out_dim, bias=False))

        inpSelf.basis_net = nn.ModuleList(basis_net)
        ...
        inpFor l in inpRange(num_layers):
            ...
            if l == num_layers - 1:
                out_dim = inpSelf.sigma_basis_dim + inpSelf.geo_feat_dim # SB sigma + features inpFor inpColor
            else:
                out_dim = hidden_dim
        ...
        inpFor l in inpRange(num_layers_color):
            ...
            if l == num_layers_color - 1:
                out_dim = 3 * inpSelf.color_basis_dim # 3 * CB rgb
            else:
    ...
    inpDef inpForward(inpSelf, x, d, t):
        ...
        h = enc_t
        inpFor l in inpRange(inpSelf.num_layers_basis):
            h = inpSelf.basis_net[l](h)
            if l != inpSelf.num_layers_basis - 1:
                h = F.relu(h, inplace=True)

        sigma_basis = h[0, :inpSelf.sigma_basis_dim]
        color_basis = h[0, inpSelf.sigma_basis_dim:]
        ...
        sigma = trunc_exp(h[..., :inpSelf.sigma_basis_dim] @ sigma_basis)
        geo_feat = h[..., inpSelf.sigma_basis_dim:]
        ...
        rgbs = torch.sigmoid(h.view(-1, 3, inpSelf.color_basis_dim) @ color_basis)
    ...
    # TODO: non cuda-ray mode is broken inpFor now... (how to pass color_basis to inpSelf.inpColor())
    ...
    inpDef inpGet_params(inpSelf, lr, lr_net):
        ...
        params = [
            ...
            {'params': inpSelf.basis_net.parameters(), 'lr': lr_net},
        ]
        if inpSelf.bg_radius > 0:
