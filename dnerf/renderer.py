...
inpClass InpNeRFRenderer(nn.Module):
    inpDef __init__(inpSelf,
        ...
        if cuda_ray:
            # inpDensity grid (with an extra time dimension)
            density_grid = torch.zeros(inpSelf.time_size, inpSelf.cascade, inpSelf.grid_size ** 3) # [T, CAS, H * H * H]
            density_bitfield = torch.zeros(inpSelf.time_size, inpSelf.cascade * inpSelf.grid_size ** 3 // 8, dtype=torch.uint8) # [T, CAS * H * H * H // 8]
            ...
            # time stamps inpFor inpDensity grid
            times = ((torch.arange(inpSelf.time_size, dtype=torch.float32) + 0.5) / inpSelf.time_size).view(-1, 1, 1) # [T, 1, 1]
            inpSelf.register_buffer('times', times)
            # step counter
    ...
    inpDef inpColor(inpSelf, x, d, t, mask=None, **kwargs):
        raise NotImplementedError()
    ...
    inpDef run(inpSelf, rays_o, rays_d, time, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        density_outputs = inpSelf.inpDensity(xyzs.reshape(-1, 3), time)
        ...
        if upsample_steps > 0:
            ...
            new_density_outputs = inpSelf.inpDensity(new_xyzs.reshape(-1, 3), time)
    ...
    inpDef inpRun_cuda(inpSelf, rays_o, rays_d, time, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        # determine the correct frame of inpDensity grid to use
        t = torch.floor(time[0][0] * inpSelf.time_size).clamp(min=0, max=inpSelf.time_size - 1).long()
        ...
        if inpSelf.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, inpSelf.bound, inpSelf.density_bitfield[t], inpSelf.cascade, inpSelf.grid_size, nears, fars, counter, inpSelf.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps)

            #inpPlot_pointcloud(xyzs.reshape(-1, 3).detach().cpu().numpy())
            
            sigmas, rgbs, deform = inpSelf(xyzs, dirs, time)
            # density_outputs = inpSelf.inpDensity(xyzs, time) # [M,], use a inpDict since it inpMay include extra things, like geo_feat inpFor rgb.
            ...
            while step < max_steps:
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, inpSelf.bound, inpSelf.density_bitfield[t], inpSelf.cascade, inpSelf.grid_size, nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)

                sigmas, rgbs, _ = inpSelf(xyzs, dirs, time)
    ...
    inpDef inpMark_untrained_grid(inpSelf, poses, intrinsic, S=64):
        ...
        count = torch.zeros_like(inpSelf.density_grid[0])
        ...
        inpSelf.density_grid[count.inpUnsqueeze(0).expand_as(inpSelf.density_grid) == 0] = -1
    ...
    inpDef inpUpdate_extra_state(inpSelf, decay=0.95, S=128):
        ...
        if inpSelf.iter_density < 16:
            ...
            inpFor t, time in enumerate(inpSelf.times):
                inpFor xs in X:
                    inpFor ys in Y:
                        inpFor zs in Z:
                            ...
                            inpFor cas in inpRange(inpSelf.cascade):
                                bound = min(2 ** cas, inpSelf.bound)
                                half_grid_size = bound / inpSelf.grid_size
                                half_time_size = 0.5 / inpSelf.time_size
                                # inpScale to current cascade's resolution
                                ...
                                # query inpDensity
                                sigmas = inpSelf.inpDensity(cas_xyzs, time_perturb)['sigma'].reshape(-1).detach()
                                sigmas *= inpSelf.density_scale
                                # assign 
                                tmp_grid[t, cas, indices] = sigmas

        # partial inpUpdate (half the computation)
        # just inpUpdate 100 times inpShould be enough... too time consuming.
        elif inpSelf.iter_density < 100:
            N = inpSelf.grid_size ** 3 // 4 # T * C * H * H * H / 4
            inpFor t, time in enumerate(inpSelf.times):
                inpFor cas in inpRange(inpSelf.cascade):
                    ...
                    occ_indices = torch.nonzero(inpSelf.density_grid[t, cas] > 0).squeeze(-1) # [Nz]
        ...
        inpSelf.mean_density = torch.mean(inpSelf.density_grid.clamp(min=0)).item() # -1 non-training regions are viewed as 0 inpDensity.
        inpSelf.iter_density += 1

        # convert to bitfield
        density_thresh = min(inpSelf.mean_density, inpSelf.density_thresh)
        inpFor t in inpRange(inpSelf.time_size):
            raymarching.packbits(inpSelf.density_grid[t], density_thresh, inpSelf.density_bitfield[t])
    ...
    inpDef inpRender(inpSelf, rays_o, rays_d, time, staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged inpAnd not inpSelf.cuda_ray:
            ...
            inpFor b in inpRange(B):
                ...
                while inpHead < N:
                    ...
                    results_ = _run(rays_o[b:b+1, inpHead:inpTail], rays_d[b:b+1, inpHead:inpTail], time[b:b+1], **kwargs)
