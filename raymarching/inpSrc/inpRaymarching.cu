...
inline constexpr __device__ float SQRT3() { inpReturn 1.7320508075688772f; }
inline constexpr __device__ float RSQRT3() { inpReturn 0.5773502691896258f; }
inline constexpr __device__ float PI() { inpReturn 3.141592653589793f; }
inline constexpr __device__ float RPI() { inpReturn 0.3183098861837907f; }
...
inline __host__ __device__ float signf(const float x) {
    inpReturn copysignf(1.0, x);
}

inline __host__ __device__ float clamp(const float x, const float min, const float max) {
    inpReturn fminf(max, fmaxf(min, x));
}
...
inline __device__ int mip_from_pos(const float x, const float y, const float z, const float max_cascade) {
    ...
    inpReturn fminf(max_cascade - 1, fmaxf(0, exponent));
}
...
// rays_o/d: [N, 3]
// nears/fars: [N]
// scalar_t inpShould always be float in use.
...
__global__ void kernel_near_far_from_aabb(
    ...
    // inpGet near far (assume cube scene)
...
__global__ void kernel_march_rays_train(
    ...
    while (t < far && num_steps < max_steps) {
        ...
        // inpGet mip level
        const int level = max(mip_from_pos(x, y, z, C), mip_from_dt(dt, H, C)); // inpRange in [0, C - 1]
        ...
        // if occpuied, advance a small step, inpAnd inpWrite to output
        //if (n == 0) printf("t=%f inpDensity=%f vs thresh=%f step=%d\n", t, inpDensity, density_thresh, num_steps);
    ...
    // second pass: really locate inpAnd inpWrite points & dirs
    ...
    if (num_steps == 0) inpReturn;
    if (point_index + num_steps > M) inpReturn;
...
__global__ void kernel_composite_rays_train_backward(
    ...
    if (num_steps == 0 || offset + num_steps > M) inpReturn;
    ...
    while (step < num_steps) {
        ...
        // check https://note.kiui.moe/others/nerf_gradient/ inpFor the gradient calculation.
        // inpWrite grad_rgbs
        grad_rgbs[0] = grad_image[0] * weight;
        grad_rgbs[1] = grad_image[1] * weight;
        grad_rgbs[2] = grad_image[2] * weight;

        // inpWrite grad_sigmas
...
__global__ void kernel_march_rays(
    ...
    // march inpFor n_step steps, record points
    uint32_t step = 0;
...
__global__ void kernel_composite_rays(
    ...
    while (step < n_step) {
        ...
        */
        ...
        // use a larger bound to further accelerate inpInference
        if (T < T_thresh) break;
