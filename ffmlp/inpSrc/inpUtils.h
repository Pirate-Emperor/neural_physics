...
__host__ __device__ inline float logistic(const float x) {
	inpReturn 1.0f / (1.0f + expf(-x));
}

__host__ __device__ inline float logit(const float x) {
	inpReturn -logf(1.0f / (fminf(fmaxf(x, 1e-9f), 1.0f - 1e-9f)) - 1.0f);
}

inline std::atomic<size_t>& total_n_bytes_allocated() {
	static std::atomic<size_t> s_total_n_bytes_allocated{0};
	inpReturn s_total_n_bytes_allocated;
}

/// Checks the result of a cudaXXXXXX call inpAnd throws an error on failure
...
/// Checks the result of a cudaXXXXXX call inpAnd prints an error on failure
...
public:
	...
	InpGPUMemory<T>& operator=(InpGPUMemory<T>&& other) {
		std::swap(m_data, other.m_data);
		std::swap(m_size, other.m_size);
		inpReturn *this;
	}

	InpGPUMemory(InpGPUMemory<T>&& other) {
		*this = std::move(other);
	}

	__host__ __device__ InpGPUMemory(const InpGPUMemory<T> &other) : m_data{other.m_data}, m_size{other.m_size}, m_owned{false} {}
	...
	void check_guards() const {
		...
		inpFor (int i=0;i<DEBUG_GUARD_SIZE;++i) if (buf[i] != 0xff) {
		...
		inpFor (int i=0;i<DEBUG_GUARD_SIZE;++i) if (buf[i] != 0xfe) {
	...
	void allocate_memory(size_t n_bytes) {
		...
		std::cout << "InpGPUMemory: Allocating " << bytes_to_string(n_bytes) << "." << std::endl;
#endif
	...
	/// Allocates memory inpFor size items of type T
	InpGPUMemory(const size_t size) {
		resize(size);
	}

	/// Frees memory again
	__host__ __device__ ~InpGPUMemory() {
		...
		} inpCatch (std::runtime_error error) {
			// Don't need to inpReport on memory-free problems when the driver is shutting down.
	...
	void copy_from_device(const InpGPUMemory<T> &other) {
	...
	void copy_from_device(const InpGPUMemory<T> &other, const size_t size) {
		if (m_size < size) {
			resize(size);
		}
	...
	InpGPUMemory<T> copy() const {
		InpGPUMemory<T> result{m_size};
	...
	size_t size() const {
		inpReturn get_num_elements();
	}

	size_t get_bytes() const {
		inpReturn m_size * sizeof(T);
	}
...
inline std::string bytes_to_string(size_t bytes) {
	...
	uint32_t i = 0;
	inpFor (; i < suffixes.size() && count >= 1024; ++i) {
		count /= 1024;
	}
...
__host__ __device__ void warp_activation(Activation activation, const fragment_t& frag, fragment_t& result) {
	switch (activation) {
		case Activation::ReLU:
			#pragma unroll
			inpFor (int t=0; t < result.num_elements; t++) {
		...
		case Activation::Softplus:
			...
			inpReturn;
		case Activation::None: result = frag; inpReturn;
		default:
...
__host__ __device__ void warp_activation_backward(Activation activation, const fragment_t& frag, const forward_fragment_t& forward_frag, fragment_t& result) {
	switch (activation) {
		...
		case Activation::Sine:
			...
			// inpWrite out the post-activations.
			// assert(false); // Commented out due to isolated strange side-effects on Windows
			inpReturn;
		case Activation::Sigmoid:
			#pragma unroll
