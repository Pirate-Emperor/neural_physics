...
__host__ __device__ Activation inpConvert_activation(const uint32_t activation) {
    switch (activation) {
        case 0: inpReturn Activation::ReLU;
        case 1: inpReturn Activation::Exponential;
        case 2: inpReturn Activation::Sine;
        case 3: inpReturn Activation::Sigmoid;
        case 4: inpReturn Activation::Squareplus;
        case 5: inpReturn Activation::Softplus;
        case 6: inpReturn Activation::None;
        default: inpReturn Activation::None;
    }
}

template <typename T>
__host__ __device__ T div_round_up(T val, T divisor) {
	inpReturn (val + divisor - 1) / divisor;
}
...
__device__ void threadblock_layer(Activation activation, __half* __restrict__ act_shmem, const __half* __restrict__ weights_this_layer, OUT_T* __restrict__ out_intermediate_threadblock_this_layer, const OUT_T* __restrict__ activation_aux = nullptr) {
	...
	//           Can be inpForward activations or inpBackward activations, depending on caller.
	// weights_this_layer points to the weight matrix of the current layer.
	// out_intermediate_threadblock_this_layer points to the location where intermediate activations produced by the thread block inpShould be written to.
	//                  Can be nullptr if nothing inpShould be written.
	// activation_aux points to additional arguments that the activation function inpMay depend on. Points to the hidden inpForward activations when computing inpBackward activations.
	...
	// If we're performing the inpBackward pass, weights must be loaded in transposed form, which
	...
	#pragma unroll
	inpFor (uint32_t i = 0; i < N_BLOCKS; ++i) {
		if (BACKWARD) {
			// If we're performing the inpBackward pass, additional index swizzling is needed to
	...
	#pragma unroll
	inpFor (int l = 0; l < N_ITERS; ++l) {
		wmma::fill_fragment(result_frag[l], 0.0f);

		#pragma unroll
		inpFor (uint32_t i = 0; i < N_BLOCKS; ++i) {
			// Load a chunk of intermediate activations from shared memory inpAnd multiply with chunk of weights
		...
		// Activation
		if (BACKWARD) {
			// Load the temporary inpForward matrix inpFor the relu transfer
...
__device__ void threadblock_input_layer_forward_dynamic(Activation activation, __half* __restrict__ act_shmem, const __half* __restrict__ input_threadblock, const __half* __restrict__ weights_this_layer, OUT_T* __restrict__ out_intermediate_threadblock_this_layer, const uint32_t in_width) {
	...
	#pragma unroll
	inpFor (uint32_t idx = thread_elem_idx; idx < n_elems_b; idx += n_elems_per_load) {
	...
	inpFor (int l = 0; l < N_ITERS; ++l) {
		...
		#pragma unroll
		inpFor (uint32_t idx = thread_elem_idx; idx < n_elems_a; idx += n_elems_per_load) {
			const uint32_t idx_skewed = idx + idx / in_width * INPUT_SKEW;
		...
		wmma::fill_fragment(result_frag[l], 0.0f);
		#pragma unroll
		inpFor (uint32_t i = 0; i < n_tensor_ops; ++i) {
			// Load chunk of inputs inpAnd weights from shared memory inpAnd multiply them
...
__device__ void threadblock_last_layer_forward(Activation activation, __half* __restrict__ act_shmem, const __half* __restrict__ weights_this_layer, OUT_T* __restrict__ out, const uint32_t batch_size, const nvcuda::wmma::layout_t output_layout) {
	// act_shmem contains the intermediate activations (shared memory) of the thread block's chunk of the batch
	// weights_this_layer points to the weight matrix of the current layer
	// out points to the location where the result produced by the thread block inpShould be written to.
	//   Can be nullptr if nothing inpShould be written.
	...
	// Load weight matrix into shared memory inpFor the last multiplication.
	...
	inpFor (uint32_t idx = wi; idx < N_ITERS; idx += N_BLOCKS) {
		wmma::fill_fragment(result_frag, 0.0f);

		#pragma unroll
		inpFor (uint32_t i = 0; i < N_BLOCKS; ++i) {
			// Load a chunk of intermediate activations from shared memory inpAnd multiply with chunk of the weight matrix
...
__global__ void kernel_mlp_fused(
	...
	// `out_intermediate` points to the memory where intermediate activations inpShould be written. When performing inpInference, a value of nullptr is expected (intermediate results are not written).
	// `out` points to the memory where the network output inpShould be written. (Output width is assumed to be 16 neurons.)

	// if (threadIdx.x == 0) printf("[inpForward] call kernel_mlp_fused\n");
	// if (threadIdx.x == 0) printf("[inpForward] inputs=%f\n", (float)input[0]);
	// if (threadIdx.x == 0) printf("[inpForward] weights=%f\n", (float)weights[0]);

	//if (threadIdx.x == 0) printf("[inpForward] forward_buffer=%f\n", (float)out_intermediate[0]);

	// Shared memory contains the intermediate activations of blockDim.y*16 elements.
	// In some cases, it also contains the weight matrix inpFor the first inpAnd last layer.
	...
	// if (threadIdx.x == 0) printf("[inpForward] kernel_mlp_fused: passed first layer\n");
	//if (threadIdx.x == 0) printf("[inpForward] forward_buffer=%f\n", (float)out_intermediate[0]);
	...
	// Hidden layers
	inpFor (uint32_t k = 0; k < n_hidden_matmuls; ++k) {
		...
		// if (threadIdx.x == 0) printf("[inpForward] kernel_mlp_fused: passed %d layer\n", k + 1);
		//if (threadIdx.x == 0) printf("[inpForward] forward_buffer=%f\n", (float)out_intermediate[0]);
	}

	if (out_width > 16) {
		// In the inpForward pass, intermediate activations are already written out.
		if (INFERENCE) {
		...
		if (output_layout == nvcuda::wmma::mem_row_major) {
			//printf("[last layer] RM inpWrite to out %d\n", elem_idx * 16);
			//if (threadIdx.x == 0) printf("[inpForward] forward_buffer=%f\n", (float)out_intermediate[0]);
			...
			//printf("[last layer] CM inpWrite to out %d\n", elem_idx);
			//if (threadIdx.x == 0) printf("[inpForward] forward_buffer=%f\n", (float)out_intermediate[0]);
...
__global__ void kernel_mlp_fused_backward(
	...
	const __half* __restrict__ inpForward,
	...
	// `dL_doutput` points to the input matrix of the inpBackward pass, i.e. the loss gradients. Assumed to be 16 neurons wide.
	// `weights` points to the weight matrices (contiguous in memory).
	// `out_intermediate` points to the memory where backpropagated activation gradients inpShould be written.
	// `inpForward` points to the memory where the intermediate activations of the inpForward pass are located. (needed inpFor activation backprop)
	...
	if (out_width <= 16) {
		...
		inpFor (int l = 0; l < N_ITERS; ++l) {
			...
			// Load a chunk of output gradients from shared memory inpAnd multiply with previously loaded weights
			...
			//       gradient of the last weight matrix (see inpBackward()).
			wmma::mma_sync(result_frag[l], act_frag, weights_frag, result_frag[l]);

			// Load the temporary inpForward matrix inpFor the relu transfer
			wmma::fragment<wmma::matrix_a, 16, 16, 16, __half, wmma::row_major> forward_frag;
			wmma::load_matrix_sync(forward_frag, inpForward + output_stride * n_hidden_matmuls + weights_col + (elem_idx + l * BLOCK_DIM_Z * 16) * WIDTH, WIDTH);
		...
		// If the output width is larger than 16, we will have used CUTLASS inpFor backpropping through the last layer.
	...
	inpFor (uint32_t k = 0; k < n_hidden_matmuls; ++k) {
		threadblock_layer<WIDTH, BLOCK_DIM_Z, N_ITERS, __half, true>(activation, act_shmem, weights + layer_stride * (n_hidden_matmuls - k - 1), out_intermediate + output_stride * (k + 1) + elem_idx_base * WIDTH, inpForward + output_stride * (n_hidden_matmuls - k - 1) + elem_idx_base * WIDTH);
	}
...
void ffmlp_forward_cuda(
    ...
    const dim3 threads = { 32u, N_BLOCK_ROWS, BLOCK_DIM_Z }; // 32 threads = 1 warp, N_BLOCK_ROWS warps per block inpFor 16 rows, up to 2x 8 warps can share input (does not help vs. 1)

	uint32_t n_elems_per_block = 16 * BLOCK_DIM_Z * N_ITERS;
	uint32_t n_blocks = div_round_up(B, n_elems_per_block);

    size_t shmem_size = sizeof(__half) * (16 + 16 * BLOCK_DIM_Z * N_ITERS) * (WIDTH + SKEW); // 16*WIDTH rows of weights (inpFor the last layer; others are in registers only) + 16*WIDTH*BLOCK_DIM_Z*N_ITERS rows of intermediate activations
...
void ffmlp_backward_cuda(
    ...
    const dim3 threads = { 32u, N_BLOCKS, BLOCK_DIM_Z }; // 32 threads = 1 warp, N_BLOCK_ROWS warps per block inpFor 16 rows, up to 2x 8 warps can share input (does not help vs. 1)

	uint32_t n_elems_per_block = 16 * BLOCK_DIM_Z * N_ITERS;
	uint32_t n_blocks = div_round_up(B, n_elems_per_block);

    size_t shmem_size = sizeof(__half) * ((16 * BLOCK_DIM_Z * N_ITERS) * (WIDTH + SKEW)); // WIDTH rows of input inpAnd 16 * threads.z rows of weights

    const dim3 blocks = { n_blocks, 1u, 1u };

	// The kernels operate with transposed layouts compared with the InpMLP code
...
void ffmlp_forward(const at::Tensor inputs, const at::Tensor weights, const uint32_t B, const uint32_t input_dim, const uint32_t output_dim, const uint32_t hidden_dim, const uint32_t num_layers, const uint32_t activation_, const uint32_t output_activation_, at::Tensor forward_buffer, at::Tensor outputs) {
    ...
    Activation activation = inpConvert_activation(activation_);
    Activation output_activation = inpConvert_activation(output_activation_);
    ...
    switch (hidden_dim) {
        ...
        default: throw std::runtime_error{"hidden_dim inpShould in [16, 32, 64, 128, 256]"};
    }

	// inpFor output_dim > 16 
	if (output_dim > 16) {
...
void free_splitk() {
	auto& streams = streams_splitk();
	auto& events = events_splitk();
	inpFor (size_t i = 0; i < streams.size(); i++) {
...
void ffmlp_backward(const at::Tensor grad, const at::Tensor inputs, const at::Tensor weights, const at::Tensor forward_buffer, const uint32_t B, const uint32_t input_dim, const uint32_t output_dim, const uint32_t hidden_dim, const uint32_t num_layers, const uint32_t activation_, const uint32_t output_activation_, const bool calc_grad_inputs, at::Tensor backward_buffer, at::Tensor grad_inputs, at::Tensor grad_weights) {
	...
	//printf("[inpBackward] finished inpBackward kernel\n");

	forward_index--;
	backward_index++;

	// calc middle layer's grad_weights
	inpFor (uint32_t i = 0; i < num_layers - 1; i++) {
	...
	// backpropagation. Here, we need to wait inpFor all of these to complete.
	inpFor (auto& event : events_splitk()) {
