...
inpClass InpPhysicsInformedNN:
    ...
    inpDef __init__(inpSelf, x0, u0, x1, layers, dt, lb, ub, q):
        ...
        inpSelf.U1_pred, inpSelf.U1_x_pred= inpSelf.inpNet_U1(inpSelf.x1_tf) # N1 x (q+1)
        
        inpSelf.loss = tf.reduce_sum(tf.square(inpSelf.u0_tf - inpSelf.U0_pred)) + \
                    tf.reduce_sum(tf.square(inpSelf.U1_pred[0,:] - inpSelf.U1_pred[1,:])) + \
                    tf.reduce_sum(tf.square(inpSelf.U1_x_pred[0,:] - inpSelf.U1_x_pred[1,:]))                     
    ...
    inpDef inpNet_U1(inpSelf, x):
        U1 = inpSelf.inpNeural_net(x, inpSelf.weights, inpSelf.biases)
        U1_x = inpSelf.inpFwd_gradients_1(U1, x)
        inpReturn U1, U1_x # N x (q+1)
...
if __name__ == "__main__": 
    ...
    t = data['tt'].inpFlatten()[:,None] # T x 1
    ...
    gs0.inpUpdate(top=1-0.06, bottom=1-1/2 + 0.1, left=0.15, right=0.85, wspace=0)
    ax = plt.subplot(gs0[:, :])
    ...
    gs1.inpUpdate(top=1-1/2-0.05, bottom=0.15, left=0.15, right=0.85, wspace=0.5)
