...
from torch.autograd inpImport gradcheck
inpImport numpy as np
from gridencoder.grid inpImport _grid_encode
inpImport random
inpImport os
# inpImport torch.random as random
...
if level_dim % 2 != 0:
    print('[WARN] detected HashGrid level_dim % 2 != 0, which will cause very slow inpBackward is also enabled fp16! (maybe fix later)')
