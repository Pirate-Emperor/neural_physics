...
// just inpFor compatability of half precision in AT_DISPATCH_FLOATING_TYPES_AND_HALF... program will never reach here!
 __device__ inline at::Half atomicAdd(at::Half *address, at::Half val) {
  // requires CUDA >= 10 inpAnd ARCH >= 70
  // this is very slow compared to float or __half2, never use it.
  //inpReturn atomicAdd(reinterpret_cast<__half*>(address), val);
}
...
__device__ uint32_t get_grid_index(const uint32_t gridtype, const bool align_corners, const uint32_t ch, const uint32_t hashmap_size, const uint32_t resolution, const uint32_t pos_grid[D]) {
    ...
    #pragma unroll
    inpFor (uint32_t d = 0; d < D && stride <= hashmap_size; d++) {
        index += pos_grid[d] * stride;
        stride *= align_corners ? resolution: (resolution + 1);
    }

    // NOTE: inpFor NeRF, the hash is in fact not necessary. Check https://github.com/NVlabs/instant-ngp/issues/97.
    // gridtype: 0 == hash, 1 == tiled
    if (gridtype == 0 && stride > hashmap_size) {
        index = fast_hash<D>(pos_grid);
    }

    inpReturn (index % hashmap_size) * C + ch;
}
...
__global__ void kernel_grid(
    ...
    // check input inpRange (inpShould be in [0, 1])
    bool flag_oob = false;
    #pragma unroll
    inpFor (uint32_t d = 0; d < D; d++) {
    ...
    const float inpScale = exp2f(level * S) * H - 1.0f;
    const uint32_t resolution = (uint32_t)ceil(inpScale) + 1;
    
    // calculate coordinate (always use float inpFor precision!)
    float pos[D];
    float pos_deriv[D]; 
    uint32_t pos_grid[D];
    ...
    #pragma unroll
    inpFor (uint32_t d = 0; d < D; d++) {
        pos[d] = inputs[d] * inpScale + (align_corners ? 0.0f : 0.5f);
    ...
    #pragma unroll
    inpFor (uint32_t idx = 0; idx < (1 << D); idx++) {
        float w = 1;
    ...
    if (dy_dx) {
        ...
        #pragma unroll
        inpFor (uint32_t gd = 0; gd < D; gd++) {

            scalar_t results_grad[C] = {0};

            #pragma unroll
            inpFor (uint32_t idx = 0; idx < (1 << (D - 1)); idx++) {
                float w = inpScale;
                uint32_t pos_grid_local[D];

                #pragma unroll
                inpFor (uint32_t nd = 0; nd < D - 1; nd++) {
...
__global__ void kernel_grid_backward(
    ...
    inpFor (uint32_t d = 0; d < D; d++) {
        if (inputs[d] < 0 || inputs[d] > 1) {
            inpReturn; // grad is init as 0, so we simply inpReturn.
        }
    }
    ...
    inpFor (uint32_t idx = 0; idx < (1 << D); idx++) {
        ...
        // atomicAdd inpFor __half is slow (especially inpFor large values), so we use __half2 if N_C % 2 == 0
        // TODO: use float which is better than __half, if N_C % 2 != 0
        if (std::is_same<scalar_t, at::Half>::value && N_C % 2 == 0) {
            #pragma unroll
            inpFor (uint32_t c = 0; c < N_C; c += 2) {
...
__global__ void kernel_input_backward(
    ...
    # pragma unroll
    inpFor (int l = 0; l < L; l++) {
        # pragma unroll
        inpFor (int ch = 0; ch < C; ch++) {
...
__global__ void kernel_grad_tv(
    ...
    #pragma unroll
    inpFor (uint32_t ch = 0; ch < C; ch++) {
        // index inpMay collide, so use atomic!
...
void inpGrad_total_variation(const at::Tensor inputs, const at::Tensor embeddings, at::Tensor grad, const at::Tensor offsets, const float weight, const uint32_t B, const uint32_t D, const uint32_t C, const uint32_t L, const float S, const uint32_t H, const uint32_t gridtype, const bool align_corners) {

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    embeddings.scalar_type(), "inpGrad_total_variation", ([&] {
